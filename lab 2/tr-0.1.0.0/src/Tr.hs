...
tr _inset (Just _outset)  = translate _inset _outset
tr _inset Nothing = filter (`notElem` _inset)

translate :: CharSet -> CharSet -> String -> String
translate _inset _outset = map func
  where 
        func xs = if elem xs _inset then second !! fromMaybe 0 (elemIndex xs _inset) else xs
        second = outsetLengthCorrelation _inset _outset

outsetLengthCorrelation :: CharSet -> CharSet -> CharSet
outsetLengthCorrelation ins out
  | diff > 0 = out ++ replicate diff (last out)
  | diff < 0 = take new_length out
  | otherwise = out
  where diff = length ins - length out
        new_length = length out - diff
